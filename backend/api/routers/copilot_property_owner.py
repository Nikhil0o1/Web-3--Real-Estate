"""Property-owner AI Copilot — conversational orchestration (Phase 5)."""
...
router = APIRouter(prefix="/copilot/property-owner", tags=["property-owner-copilot"])
...
async def property_owner_copilot_chat(
    ...
    if body.thread_id is not None:
        ...
        tid = store.create_thread(
            ...
            platform_role=role,
            title=title,
            metadata={"source": "property_owner_copilot", "trace_id": trace},
        )
    ...
    raw = await get_orchestration_service().property_owner_copilot_turn(
        ctx,
        db,
...
async def _sse_property_owner_copilot(
    *,
    body: InvestorCopilotChatRequest,
    user: AuthUser,
    db: Any,
    ...
    if body.thread_id is not None:
        ...
        tid = store.create_thread(
            ...
            platform_role=role,
            title=title,
            metadata={"source": "property_owner_copilot_stream", "trace_id": trace},
        )
    store.append_message(thread_id=tid, author="user", content=body.message, event_payload={"trace_id": trace})
    tail = store.list_messages(thread_id=tid, limit=40)
...
async def property_owner_copilot_chat_stream(
    body: InvestorCopilotChatRequest,
    user: AuthUser = Depends(require_property_owner),
    db=Depends(get_db),
):
    ...
    gen = _sse_property_owner_copilot(body=body, user=user, db=db)
